...
/*   Updated: 2024/10/09 19:06:24 by gigardin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	exit_options(char **options)
{
	...
	return (kill_all(ft_global_mini(NULL), nbr % 256));
}
...
int	execute_exit(t_mini *minishell, t_cmd *cmd)
{
	...
	ft_putendl_fd("exit", 1);
	return (kill_all(minishell, status));
}
